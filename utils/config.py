import os

WEEKLY_LIMIT_USD      = 20
COIN_COOLDOWN_SECONDS = 25

try:
    OWNER_ID = int(os.environ.get('OWNER_ID', '823310792291385424'))
except (ValueError, TypeError):
    raise RuntimeError(
        "OWNER_ID in the environment is not a valid integer. "
        "Check your .env file and make sure OWNER_ID contains only digits."
    )

try:
    MOD_ROLE_ID = int(os.environ.get('MOD_ROLE_ID', '1373312465626202222'))
except (ValueError, TypeError):
    raise RuntimeError(
        "MOD_ROLE_ID in the environment is not a valid integer. "
        "Check your .env file and make sure MOD_ROLE_ID contains only digits."
    )

DEFAULT_SHOP_PRICES = {
    'PayPal': 100, 'Steam': 100, 'Google Play': 100,
    'Apple Store': 100, 'Discord Nitro Basic': 100,
    'Discord Nitro Boost': 100, 'Nintendo Card': 100, 'Roblox': 100
}
